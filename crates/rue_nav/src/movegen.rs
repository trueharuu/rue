...
use rue_core::board::{Board, bitlen, ctz, is_ok_x, popcount};
use rue_core::coordinates::Coordinates;
use rue_core::data::{Direction, KICKS, KICKS_180, SPAWN_COL, canonical_offset, canonical_r, in_bounds, kick_180_index, kick_index, rotate};
...
use rue_core::{
    board::{Bitboard, COL_NB},
    data::{canonical_size, group2},
    rotation::ROTATION_NB,
};

use crate::collision_map::CollisionMap16;
...
fn generate_inner<const P: usize, const CHECK_SPIN: bool>(
    ...
    if slow {
        ...
        if CHECK_SPIN {
            spin_set[SPAWN_COL][Rotation::North as usize][SpinType::None as usize] = spawn;
        }
    } else {
        for x in 0..COL_NB {
            for ri in 0..canonical_sz {
                ...
                if CHECK_SPIN {
                    spin_set[x][ri][SpinType::None as usize] = surface;
                } else {
    ...
    while remaining != 0 {
        ...
        if CHECK_SPIN {
            let mut m = (to_search[x][ri] >> 1) & !cm.get(x, r) & !searched[x][ri];
            while (m & to_search[x][ri]) != m {
                to_search[x][ri] |= m;
                m |= (m >> 1) & !cm.get(x, r) & !searched[x][ri];
            }
            spin_set[x][ri][SpinType::None as usize] |= m;
        } else {
...

        {
            let mut do_shift = |x1: usize| {
                ...
                if m != 0 {
                    ...
                    if CHECK_SPIN {
                        spin_set[x1][ri][SpinType::None as usize] |= m;
                    }
                }
            };
            if x > 0 {
                    ...
                    for (i, kick) in kicks.iter().enumerate().take(n) {
                        ...
                        if CHECK_SPIN {
                            ...
                            if let Some(smap) = spin_map {
                                ...
                                spin_set[x1u][r1i][SpinType::None as usize] &= !spins;
                                spin_set[x1u][r1i][SpinType::None as usize] |= m ^ spins;
                                if spins != 0 {
                                    if i >= 4 {
                                ...
                                spin_set[x1u][r1i][SpinType::None as usize] &= !stuck;
                                spin_set[x1u][r1i][SpinType::Mini as usize] |= stuck;
                                spin_set[x1u][r1i][SpinType::None as usize] |= m ^ stuck;
                            }
                        }
    ...
    if CHECK_SPIN {
        for x in 0..COL_NB {
            for ri in 0..canonical_sz {
                ...
                let raw_nospin = legal & spin_set[x][ri][SpinType::None as usize];
    ...
    for (i, kick) in kicks.iter().enumerate().take(n) {
        ...
        if CHECK_SPIN {
            ...
            if let Some(smap) = ctx.spin_map {
                let spins = m & smap[x1u][0];
                ctx.spin_set[x1u][r1i][SpinType::None as usize] &= !spins;
                ctx.spin_set[x1u][r1i][SpinType::None as usize] |= m ^ spins;
                if spins != 0 {
                    if i >= 4 {
                ...
                ctx.spin_set[x1u][r1i][SpinType::None as usize] &= !stuck;
                ctx.spin_set[x1u][r1i][SpinType::Mini as usize] |= stuck;
                ctx.spin_set[x1u][r1i][SpinType::None as usize] |= m ^ stuck;
            }
        }
